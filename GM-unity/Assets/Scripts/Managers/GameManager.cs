...
public enum GameState { gameStarted, gamePlaying, gameEnded, gameLevelEnded, gamePaused, gameTesting };
...
public class GameManager : MonoBehaviour
{
    ...
    [HideInInspector] public GameState gameState = GameState.gameStarted; //first game state
    ...
    /*Game Events see C# Game Programming Cookbook for Unity 3D
    ...
    */
    ...
    //Update the target game state (make virtual for advance managers see C# Game Programming Cookbook for Unity 3D)
    public void UpdateTargetState()
    {
        ...
        switch (targetGameState)
        {
            ...
            case GameState.gamePlaying:
                ...
                if (Application.isEditor) { SetTargetState(GameState.gameTesting); }
                StartGame();
            ...
            case GameState.gameTesting:
                StartGame();
                SetGameDefaults(); //set the defaults
                break;
    ...
    public void UpdateCurrentState()
    {
        ...
        {
            case GameState.gameStarted:
                Debug.Log("Game is currently title");
                break;
    ...
    void SetGameDefaults()
    {
        ...
        if(!isTimed){ Timer = "no timer"; } //if there is no timer but the UI is looking for one, this is the default text
        if(!hasCollection){ Collection = "no collection"; } //if there is no collection but the UI is looking for one, this is the default text
